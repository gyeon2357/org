...
canvas.addEventListener("mousemove", (e) => {
  pointer.moved = true;
  pointer.dx = 10 * (e.pageX - pointer.x);
  pointer.dy = 10 * (e.pageY - pointer.y);
  pointer.x = e.pageX;
  pointer.y = e.pageY;
  pointer.firstMove = true;
});
    ...
    const newX =
      (0.5 +
        0.2 *
          ...
          Math.sin(0.009 * prevTimestamp)) *
      window.innerWidth;
    const newY =
      (0.5 + 0.2 * Math.sin(0.02 * prevTimestamp)) * window.innerHeight;
    pointer.dx = 30 * (newX - pointer.x);
    pointer.dy = 20 * (newY - pointer.y);
    pointer.x = newX;
    pointer.y = newY;
  }
